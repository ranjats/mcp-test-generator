...
import { getSourceExtensions, getTestFileExtension } from "./language-detector.js";
...
export function findExistingTest(
  ...
  // Python-specific
  if (language === "python") {
    possibleTestPaths.push(
      path.join(dir, `test_${nameWithoutExt}.py`),
      path.join(dir, `${nameWithoutExt}_test.py`),
      path.join("tests", `test_${nameWithoutExt}.py`)
    );
  }
  ...
  if (language === "java") {
    possibleTestPaths.push(
      path.join(dir.replace("main", "test"), `${nameWithoutExt}Test.java`)
    );
  }
