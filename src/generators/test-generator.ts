...
import { TestSuite, TestCase, DescribeBlock } from "../models/test-case.js";
import {
  GenerationConfig,
  ProgrammingLanguage,
...
export function generateTestSuite(
  ...
  for (const func of analysis.functions) {
    if (!func.isExported && !config.coverageTargets.functions) continue;
...
function generateTestCasesForFunction(
  ...
  if (func.parameters.length > 0) {
    ...
    for (const param of func.parameters) {
      if (config.coverageTargets.edgeCases) {
        testCases.push(
          ...generateEdgeCaseTests(func, param)
        );
      }

      if (config.coverageTargets.errorHandling) {
        testCases.push(
          ...generateErrorHandlingTests(func, param)
        );
      }
    }
  }
  ...
  if (func.parameters.every((p: any) => p.isOptional)) {
    testCases.push({
...
function generateTestsForClass(cls: any, config: GenerationConfig): DescribeBlock {
  const nestedDescribes: DescribeBlock[] = [];
  const classCases: TestCase[] = [];
  ...
  for (const method of cls.methods) {
    ...
    const methodDescribe: DescribeBlock = {
      name: method.name,
      testCases: generateTestCasesForFunction(
...
function generateEdgeCaseTests(func: any, param: any): TestCase[] {
...
function generateErrorHandlingTests(func: any, param: any): TestCase[] {
  const tests: TestCase[] = [];
  tests.push({
...
function generateReturnTypeTests(_func: any): TestCase[] {
  return [];
}

function generateArrangement(func: any): string {
  const args = func.parameters.map((p: any) => p.defaultValue ?? "undefined").join(", ");
  return `// Arrange\nconst result = ${func.isAsync ? "await " : ""}${func.name}(${args});`;
}

function generateAction(func: any): string {
  const args = func.parameters.map((p: any) => p.name + ": " + (p.defaultValue ?? "undefined")).join(", ");
  return func.isAsync ? `const result = await ${func.name}(${args});` : `const result = ${func.name}(${args});`;
}

function generateAssertion(_func: any): string {
  return "expect(result).toBeDefined();";
}

function generateInvalidArgs(func: any): string {
  return func.parameters.map((_p: any) => "undefined").join(", ");
}

function generateConstructorArrangement(_cls: any): string {
  return "// Arrange";
}

function generateConstructorArgs(cls: any): string {
  return (cls.constructorInfo?.parameters ?? []).map((p: any) => p.defaultValue ?? "undefined").join(", ");
}

function generateArrangementWithOverride(func: any, paramName: string, value: string): string {
  const args = func.parameters.map((p: any) => (p.name === paramName ? value : p.defaultValue ?? "undefined")).join(", ");
  return `// Arrange with ${paramName}=${value}\nconst result = ${func.isAsync ? "await " : ""}${func.name}(${args});`;
}

function generateActionWithOverride(func: any, paramName: string, value: string): string {
  const args = func.parameters.map((p: any) => (p.name === paramName ? value : p.defaultValue ?? "undefined")).join(", ");
  return func.isAsync ? `const result = await ${func.name}(${args});` : `const result = ${func.name}(${args});`;
}

function generateImportStatements(analysis: FileAnalysis, _language: ProgrammingLanguage, _config: GenerationConfig): string[] {
