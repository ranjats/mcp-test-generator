...
function extractClasses(content: string): ClassInfo[] {
  ...
  const classRegex =
    /(?:(export)\s+)?(?:(default)\s+)?class\s+(\w+)(?:\s+extends\s+(\w+))?(?:\s+implements\s+([^{]+))?\s*\{/g;
  ...
  while ((match = classRegex.exec(content)) !== null) {
    ...
    const constructorInfo =
      methods.find((m) => m.type === "constructor") || null;
...
function extractMethods(classBody: string, className: string): FunctionInfo[] {
...
function parseOneParameter(param: string): ParameterInfo {
  const isRest = param.startsWith("...");
  if (isRest) param = param.substring(3);
...
function extractJsDoc(content: string, functionIndex: number): string | null {
...
function extractDecorators(content: string, classIndex: number): string[] {
...
function extractClassProperties(classBody: string): PropertyInfo[] {
  ...
  const propRegex =
    /(?:(private|protected|public)\s+)?(?:(static)\s+)?(?:(readonly)\s+)?(\w+)(?:\?)?(?::\s*([^;=\n]+))?(?:\s*=\s*([^;\n]+))?;/g;
  ...
  while ((match = propRegex.exec(classBody)) !== null) {
    ...
    const name = match[4];
    if (
      ["if", "else", "return", "const", "let", "var", "for", "while"].includes(
        name
      )
    )
      continue;
...
function extractConstants(content: string): any[] {
  const constants: any[] = [];
  const constRegex =
  ...
  while ((match = constRegex.exec(content)) !== null) {
    ...
    if (value.includes("=>") || value.startsWith("function")) continue;
...
function extractInterfaces(content: string): any[] {
