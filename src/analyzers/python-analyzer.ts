...
function extractPythonFunctions(content: string): FunctionInfo[] {
  ...
  while ((match = funcRegex.exec(content)) !== null) {
    ...
    const docstringMatch = afterDef.match(
      /^\s*(?:"""([\s\S]*?)"""|'''([\s\S]*?)''')/
    );
...
function extractPythonClasses(content: string): ClassInfo[] {
  ...
  while ((match = classRegex.exec(content)) !== null) {
    ...
    const constructorInfo =
      methods.find((m) => m.name === "__init__") || null;
...
function extractMethodsFromPythonClass(
  classBody: string,
  className: string
): FunctionInfo[] {
  ...
  while ((match = methodRegex.exec(classBody)) !== null) {
    ...
    methods.push({
      ...
      isStatic,
      parameters: parsePythonParameters(paramsStr).filter(
...
function extractPythonDecorators(
  content: string,
  defIndex: number
): string[] {
...
function calculatePythonComplexity(body: string): number {
  let complexity = 1;
  const keywords = [
    /\bif\b/g,
    /\belif\b/g,
